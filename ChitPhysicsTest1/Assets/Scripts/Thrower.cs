...
public class Thrower : NetworkBehaviour
{
        ...
        switch (touch.phase) {
            ...
            case TouchPhase.Ended:
                ...
                if (beginTouchPosition.x <= endTouchPosition.x && !ballScript.InPlay) {
                    if(IsClient){
                        ThrowServerRpc(varZ, rotSpeed, Ball.SpinDirection.CLOCKWISE);
                    }
                    else{
                        Throw(varZ, rotSpeed, Ball.SpinDirection.CLOCKWISE); 
                    }
                }
                ...
                {
                    if(IsClient){
                        ThrowServerRpc(varZ, rotSpeed, Ball.SpinDirection.COUNTERCLOCKWISE);
                    }
                    else{
                        Throw(varZ, rotSpeed, Ball.SpinDirection.COUNTERCLOCKWISE);    
                    }
                }
    ...
    void ThrowServerRpc(float varZ, float rotSpeed, Ball.SpinDirection direction,){
        Throw(varZ, rotSpeed, Ball.SpinDirection direction)
    }
